...
import maykonPoster from './assets/testimonials/maykon.jpg';
import jessicaVideo from './assets/testimonials/jessica.mp4';
import jessicaPoster from './assets/testimonials/jessica.jpg';
import remediosVideo from './assets/testimonials/Remédios.mp4';
  ...
  return <>
    ...
    <header className="site-header"><div className="container header-inner"><Brand />
      <nav className={menu ? 'main-nav is-open' : 'main-nav'} aria-label="Navegação principal" id="main-navigation">
        <a href="#metodo" onClick={() => setMenu(false)}>O método</a><a href="#comparacao" onClick={() => setMenu(false)}>Comparação</a><a href="#para-quem" onClick={() => setMenu(false)}>Para quem</a><a href="#duvidas" onClick={() => setMenu(false)}>Dúvidas</a>
    ...
    <main id="conteudo">
      ...
      <section className="section testimonials-section" id="depoimentos" aria-labelledby="testimonials-title">
        <div className="container">
          <div className="section-heading centered reveal">
            <Label>QUEM CONFIA NA FONIL</Label>
            <h2 id="testimonials-title">Experiências reais.<br /><em>Na voz de quem viveu.</em></h2>
            <p>Conheça experiências reais de clientes sobre o trabalho da Fonil.</p>
          </div>
          <div className="testimonials-grid">
            {[{ name: 'Maykon', video: maykonVideo, poster: maykonPoster }, { name: 'Jéssica', video: jessicaVideo, poster: jessicaPoster }].map(({ name, video, poster }) => (
              <figure className="testimonial-card reveal" key={name}>
                <video controls playsInline preload="none" poster={poster} width={720} height={1280} aria-label={'Depoimento de ' + name + ' sobre a Fonil'}>
                  <source src={video} type="video/mp4" />
                  Seu navegador não suporta vídeos. <a href={video}>Assista ao depoimento de {name}.</a>
                </video>
                <figcaption><strong>{name}</strong><span>DEPOIMENTO FONIL</span></figcaption>
              </figure>
            ))}
            <figure className="testimonial-card reveal">
              <video controls playsInline preload="none" width={720} height={1280} aria-label="Depoimento de cliente sobre a Fonil">
                <source src={remediosVideo} type="video/mp4" />
                Seu navegador não suporta vídeos. <a href={remediosVideo}>Assista ao depoimento.</a>
              </video>
              <figcaption><strong>Remédios</strong><span>DEPOIMENTO FONIL</span></figcaption>
            </figure>
          </div>
        </div>
      </section>
      ...
      <section className="section comparison-section" id="comparacao">
        <div className="container">
          <div className="section-heading reveal"><Label>FAÇA A COMPARAÇÃO</Label><h2>Os mesmos representantes.<br /><span className="muted-heading">Outra velocidade de crescimento.</span></h2></div>
          <div className="comparison-grid reveal"><div className="comparison-column before"><div className="comparison-head"><CircleDot size={22} /><span>OPERAÇÃO ATUAL</span><h3>Prospecção individual</h3><p>A expansão depende de cada pessoa.</p></div>{comparisons.map(([before]) => <div className="comparison-item" key={before}><X size={16} /><span>{before}</span></div>)}</div><div className="comparison-column after"><div className="comparison-head"><Network size={22} /><span>COM O MÉTODO SAC</span><h3>Aquisição estruturada</h3><p>A expansão passa a ter um sistema.</p></div>{comparisons.map(([, after]) => <div className="comparison-item" key={after}><Check size={16} /><span>{after}</span></div>)}</div></div>
          <div className="centered-cta reveal"><CTA source="comparison" className="comparison-cta">Quero descobrir o estágio da minha empresa</CTA><span>Diagnóstico de 2 a 4 minutos</span></div>
        </div>
      </section>
